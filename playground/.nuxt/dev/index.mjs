...
function columnCountToRange(columnCount) {
...
const _5N9R6k = defineEventHandler(async (event) => {
  ...
  return {
    spreadsheetId,
    sheets: ((_a = response.sheets) != null ? _a : []).map((sheet) => {
      var _a2, _b, _c, _d, _e;
      const columnCount = (_c = (_b = (_a2 = sheet.properties) == null ? void 0 : _a2.gridProperties) == null ? void 0 : _b.columnCount) != null ? _c : 1;
      return {
        label: (_e = (_d = sheet.properties) == null ? void 0 : _d.title) != null ? _e : "",
...
function getSchemaAtPath(rootSchema, path) {
  ...
  for (const segment of path) {
    ...
    current = (_a = shape[segment]) != null ? _a : null;
  }
  return current;
}
...
function transformAndValidateRows(values, schema) {
  var _a, _b;
  if (!(values == null ? void 0 : values.length)) {
    return { records: [], errors: [] };
  }
  const headers = (_a = values[0]) != null ? _a : [];
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    const row = (_b = rows[rowIndex]) != null ? _b : [];
    const candidate = {};
...
const _2g5Nlv = defineEventHandler(async (event) => {
  ...
  const values = (_a = googleResponse.values) != null ? _a : [];
...
async function resolveCollectionTypeBySchema(schemaKey, collectionTypeBySchemaFromConfig) {
  if (!schemaKey) {
    return "unknown";
  }
...
async function writeRecordAsFrontmatter(input) {
  var _a, _b, _c, _d;
  ...
  const outputFormat = (_a = input.outputFormat) != null ? _a : "frontmatter";
  const overwriteMode = (_b = input.overwriteMode) != null ? _b : "overwrite";
  if (!targetFolder.startsWith(contentRoot)) {
    throw new Error("Invalid destination folder");
  }
  await mkdir(targetFolder, { recursive: true });
  const slug = String((_c = getByPath(input.record, input.slugKey)) != null ? _c : "").trim();
  if (!slug) {
    throw new Error(`Missing slug value for key "${input.slugKey}"`);
  }
  const order = input.orderKey ? String((_d = getByPath(input.record, input.orderKey)) != null ? _d : "").trim() : "";
...
const _a14vvZ = defineEventHandler(async (event) => {
  ...
  const resolvedContentDir = (_b = body.contentDir) != null ? _b : resolveContentDirByCollectionType(resolvedCollectionType, body.schema, moduleConfig.defaultContentDir);
  const logs = [];
  const summary = {
    written: 0,
    overwritten: 0,
